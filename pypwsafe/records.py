...
class Record:
    ...
    def setPassword(
        self,
        password,
        updatePWModified=True,
        updateEntryModified=True,
        addToHistory=True,
    ):
        if updatePWModified:
...
class RecordProp(metaclass=_RecordPropType):
    ...
    def __init__(self, ptype, plen, pdata):
        ...
        if self.rNAME == "Unknown":
            psafe_logger.debug(
                "Created psafe record prop of unknown type. rType: %s rLen: %s Data: %s Raw: %s",
                repr(ptype),
                repr(plen),
                repr(self.data),
                repr(self.raw_data),
            ...
            psafe_logger.debug(
                "Created psafe record prop of known type. rType: %s rLen: %s Data: %s Raw: %s",
                repr(ptype),
                repr(plen),
                repr(self.data),
                repr(self.raw_data),
...
class PasswordHistoryRecordProp(RecordProp):
    ...
    def parse(self):
        ...
        if len(self.data) > 0:
            ...
            psafe_logger.debug(
                "Set current size of password history to %d", self._cursize
            )
...
class ProtectedEntryRecordProp(RecordProp):
    ...
    def serial(self):
        ...
        psafe_logger.debug(
            "Serial to %s data %s for %r" % (repr(ret), repr(self.data), self.__class__)
        )
        return ret
